...
- [opskarta v3 Specification](#opskarta-v3-specification)
...
In v3, calendar planning is separated from work structure:
...
Views are representations for plan visualization. In v3, views **do not affect** schedule computation.

## Concept

| Aspect | v3 |
|--------|----|
| Calendar | `excludes` in `schedule.calendars` |
| Effect on computation | No |
| Purpose | Visualization and slices |
...
| `window_start` | string | Left Gantt window boundary (`YYYY-MM-DD`) |
| `window_finish` | string | Right Gantt window boundary (`YYYY-MM-DD`) |

### Forbidden Fields

In v3, the `excludes` field is **forbidden** in views:
...
| `x_ops_attention_class` | list[string] | Nodes with the specified `node.x.ops.attention_class` |
...
| `expand_descendants` | string | `leaves`, when listed nodes should expand to leaf descendants |

With `expand_descendants: leaves`, a lane can reference a large phase/summary and the renderer will output its leaf descendants:
...
| `window_start` | Visible window start | `2026-06-01` |
| `window_finish` | Visible window finish | `2026-06-10` |

`window_start` and `window_finish` only affect output. Source dates in `schedule.nodes` remain unchanged.
...
This section describes validation rules for v3 plan files.
