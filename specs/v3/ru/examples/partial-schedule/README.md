...
Этот пример демонстрирует ключевую возможность opskarta v3: **часть узлов запланирована на текущий спринт, а остальные остаются в бэклоге без календарных дат**.

## Что демонстрирует

### Ключевая идея: Opt-in Scheduling

В v3 только узлы, явно указанные в `schedule.nodes`, участвуют в календарном планировании. Остальные узлы:
