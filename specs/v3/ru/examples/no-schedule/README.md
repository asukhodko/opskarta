...
Этот пример демонстрирует, что план opskarta v3 может быть **валидным и полезным без блока `schedule`** — только структура, иерархия, зависимости и оценки трудозатрат (effort).

## Что демонстрирует

### Ключевая идея v3: Overlay Schedule

В v3 календарное планирование — это **опциональный слой**, который накладывается на структуру работ. План без `schedule`:
...
В v3 структура и расписание разделены:

```yaml
# v3 — nodes (структура, всегда валидна)
nodes:
  task1:
    title: "Задача"
    effort: 5

# v3 — schedule (опционально, добавляется когда нужно)
schedule:
  nodes:
    task1:
      start: "2024-03-01"
