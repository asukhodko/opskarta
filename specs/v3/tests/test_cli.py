"""
Tests for opskarta v3 CLI.

This module tests the command-line interface for validating
and rendering opskarta v3 plan files.
...
from specs.v3.tools.cli import (
    create_parser,
    main,
    cmd_validate,
    cmd_render_gantt,
    ...
    cmd_render_list,
    cmd_render_deps,
    cmd_render_executive,
    cmd_render_executive_report,
)
...
@pytest.fixture
def plan_with_executive(temp_dir: Path) -> Path:
    """Create a plan file with x.exec overlay for testing."""
    plan_file = temp_dir / "executive_plan.yaml"
    plan_file.write_text("""
version: 3
meta:
  id: executive-plan
...
x:
  exec:
    program:
      ...
      nearest_goal: "Finish the release."
    blocks:
      prep:
        title: "Preparation"
        scope_nodes: [task1]
...
class TestParser:
    ...
    def test_render_executive_parsing(self):
        """Render executive command should parse correctly."""
        parser = create_parser()
        args = parser.parse_args(["render", "executive", "plan.yaml", "--view", "exec-top"])
        assert args.command == "render"
        assert args.format == "executive"
        assert args.files == ["plan.yaml"]
        assert args.view == "exec-top"
    ...
    def test_update_markdown_parsing(self):
        ...
        parser = create_parser()
        args = parser.parse_args(["update-markdown", "plan.md", "exec.md"])
        assert args.command == "update-markdown"
        assert args.files == ["plan.md", "exec.md"]
...
class TestRenderExecutiveCommand:
    """Tests for the render executive commands."""

    def test_render_executive_basic(self, plan_with_executive: Path, capsys):
        """Render executive should produce Mermaid flowchart output."""
        result = cmd_render_executive([str(plan_with_executive)], "exec-top", None, False)
        assert result == 0
    ...
    def test_render_executive_report_status(self, plan_with_executive: Path, capsys):
        """Render executive-report should produce Markdown output."""
        result = cmd_render_executive_report([str(plan_with_executive)], "status")
        assert result == 0
    ...
    def test_render_executive_via_main(self, plan_with_executive: Path, capsys):
        ...
        result = main(["render", "executive", str(plan_with_executive), "--view", "exec-top"])
        assert result == 0
