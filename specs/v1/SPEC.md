...
1. **Если указаны `finish` + `duration` без `start`:**
   ...
   - Пример: `finish: 2024-03-15`, `duration: 5d` → `start: 2024-03-08` (5 рабочих дней до 15-го).
...
nodes:
  release_prep:
    ...
    duration: "5d"
    # start вычисляется назад: 2024-03-08 (5 рабочих дней до 03-15)
...
3. **Зависимости `after`** (если `start` и `finish` отсутствуют): вычислить `start = next_workday(max_finish_зависимостей)`.
...
| Цепочка `after` без якоря (нет `start`/`finish` в цепочке) | error |
...
Цепочка узлов, связанных зависимостями `after`, ДОЛЖНА иметь хотя бы один якорь (узел с явным `start` или `finish`). Иначе цепочка не может быть распланирована.

**Условие ошибки:** если при рекурсивном обходе зависимостей `after` ни один узел не имеет `start` или `finish`, валидация ДОЛЖНА завершиться ошибкой.

```yaml
# ОШИБКА: цикл без якоря
nodes:
  task1:
    title: "Задача 1"
    after: [task2]
  ...
  task2:
    title: "Задача 2"
    after: [task1]  # Циклическая зависимость, нет якоря
    duration: "2d"
```

```yaml
# ОШИБКА: цепочка без якоря
nodes:
  task1:
    title: "Задача 1"
    after: [task2]
...
**Эта валидация ДОЛЖНА выполняться при валидации плана, а не только при рендеринге.**
